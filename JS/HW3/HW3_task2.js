...
//option 1
var uniqueUsers = usersContactInfo.filter((user, i, arr) => arr.map(user => user.name).indexOf(user.name) == i )
console.log(uniqueUsers);


//option 2
var uniqueUsers2 = [... new Set( usersContactInfo.map(user => user.name))].map( name => {return usersContactInfo.find( user => user.name === name )});
...
let usersContactInfo2 = fs.readFileSync('task2.json');
let userInfo = JSON.parse(usersContactInfo2)
var uniqueUsers3 = userInfo.filter((user, i, arr) => arr.map(user => user.name).indexOf(user.name) == i )
